...
export function applyStudioLights(scene: THREE.Scene, origin: THREE.Vector3): void {
  const hemi = new THREE.HemisphereLight(0xf1f3f5, 0x484744, 1.25);
  scene.add(hemi);
  scene.add(new THREE.AmbientLight(0xd8dcdf, 0.34));

  const key = new THREE.DirectionalLight(0xfff7eb, 0.82);
  key.position.set(origin.x - 2.6, origin.y + 5.4, origin.z - 2.0);
  key.castShadow = true;
  key.shadow.mapSize.set(1024, 1024);
  scene.add(key);

  const fill = new THREE.DirectionalLight(0xd5dfec, 0.5);
  fill.position.set(origin.x + 3.4, origin.y + 2.2, origin.z - 0.6);
  scene.add(fill);

  const rim = new THREE.DirectionalLight(0xf1ece3, 0.62);
