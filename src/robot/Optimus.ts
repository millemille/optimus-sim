...
export class Optimus {
  ...
  private poseIdle(w: number): void {
    ...
    this.lElbow.rotation.x = THREE.MathUtils.lerp(this.lElbow.rotation.x, 0.14, w);
    this.rElbow.rotation.x = THREE.MathUtils.lerp(this.rElbow.rotation.x, 0.14, w);
  ...
  private buildHead(): void {
    ...
    head.name = "head";
    head.position.set(0, 0.545, 0.01);
    this.chest.add(head);
  ...
  private arm(side: number): THREE.Group {
    ...
    socket.position.set(side * 0.032, -0.03, 0);
    shoulder.add(socket);
  ...
  private hand(side: number): THREE.Group {
    ...
    const plate = this.panel(0.054, 0.042, 0.007, this.mats.white, 0.004);
    plate.position.set(0, -0.014, 0.012);
    g.add(plate);
  ...
  private leg(side: number): THREE.Group {
    ...
    hip.position.set(side * 0.088, -0.028, 0);
    this.hips.add(hip);

    const ball = this.mesh(new THREE.SphereGeometry(0.04, 14, 12), this.mats.joint);
    hip.add(ball);
    ...
    const disk = this.mesh(new THREE.CylinderGeometry(0.038, 0.038, 0.07, 18), this.mats.joint);
